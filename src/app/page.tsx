...
export default async function PortalHomePage() {
  const { properties, settings } = await getPortalData();
    ...
    <div className="bg-background">
      <section className="relative h-[70vh] min-h-[500px] max-h-[700px] w-full flex items-center justify-center">
        ...
        <div className="relative z-20 flex flex-col items-center justify-center h-full text-center text-white px-4 md:px-6">
           <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold font-headline text-primary mb-4">{settings?.homepageTitle}</h1>
           <p className="text-lg sm:text-xl md:text-2xl max-w-3xl">{settings?.homepageSubtitle}</p>
          <Button asChild size="lg" className="mt-8">
            <Link href="/properties">
              {settings?.homepageHeroButtonText} <ArrowRight className="ml-2" />
            </Link>
          </Button>
        </div>
      </section>
